...
Code(V2)의 ML.py 대비 변경점 — 과거 캠페인(364개) 데이터 분석 근거로 결정함
  ...
  2) max_temp도 목적함수/종료판정에 포함 (단, 큰 영향은 없을 것으로 예상)
     364개의 전혀 다른 형상에서도 최고온도 변동폭이 1.6%(1.93°C)뿐이라
     이번에 스윕하는 유로 8변수의 영향권 밖(방열핀 설계가 지배적인 지표)으로
     보이지만, 그래도 공식적으로 같이 추적하고 싶다는 판단 하에 다시 포함함.
     변동폭이 작아 GPR이 금방 정확히 맞히고(과거 데이터 기준 오차 대부분 0%대)
     종료판정·적응샘플링 어느 쪽도 사실상 병목이 되지 않을 것으로 예상됨.
  3) 종료기준 오차 0.5% → 3%
...
V1(2변수) 대비 핵심 변경점(V2에서 이어짐)
  ...
  3) 목적함수 5개(pressure_drop, vel_cv_pass1, vel_cv_pass2, temp_std, max_temp) + 제약조건 2개
"""
import os
import numpy as np
import pandas as pd
...
# ── 적응 샘플링 설정 (8차원 대응) ──────────────────────────────
#   N_CAND: 8차원에서는 후보를 아무리 늘려도 촘촘함이 N^(1/8)로만 개선되는
#   차원의 저주가 있음 — 32768→262144(8배)면 후보 간 평균 간격이 약 23% 줄어듦.
#   라운드당 GPR 재학습(모델 7개) 시간이 이미 수십 초 이상이라, 후보평가에
#   드는 추가 시간(+20초 안팎)은 부담 없는 수준이라 8배로 올림.
N_CAND         = 262144  # 매 회차 σ를 평가할 후보점 수 (Sobol 균형성 위해 2^18)
MIN_DIST_NORM  = 0.05    # 정규화 공간에서 기존 실험점과 이 거리 미만이면 후보 제외

# ── 목적함수 (확정 — 5개) ──────────────────────────────────────
#    이름과 log 변환 여부만 여기서 바꾸면 나머지 코드는 그대로 동작함.
#    5개 다 GPR 학습 + 적응샘플링(σ) + 종료판정에 사용:
#      - pressure_drop  : 차압
...
#                         서로 중복이 아님
#      - max_temp       : 8채널 최고온도 — 변동폭이 1.6%로 작아 이번 8변수 스윕의
#                         영향권 밖으로 보이지만, 공식 추적을 위해 포함함. 변동폭이
#                         작은 만큼 GPR이 금방 맞히고 종료판정 병목도 되지 않을 것으로 예상
OBJECTIVES = [
