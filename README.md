...
>
> 발열채널(`N_SOURCE`)도 이 형상 조정에 맞춰 8→9개로 늘었다(`icepak.py`). 위치는
> 기존 배치(`base_x=-152.1mm`, `step=41.6mm`)에서 시작점만 step의 절반(20.8mm)
> 왼쪽으로 옮기고 같은 간격으로 9개를 채운 것(`base_x=-172.9mm`) — 냉각판뒷면
> 도면(`냉각판뒷면_수정.PNG`)의 빨간 표시 위치를 좌표로 역산해서 맞췄다. 발열량은
> 채널당 동일(각 40W, 총 발열량 증가), 박스 치수도 기존과 동일(`13.3×183×8mm`).
...
> 하한만 15→13으로 내렸다 — 위와 같은 계열의 형상 조정(양 끝 발열채널 추가) 반영,
> 상한(35mm, 선호 방향)은 그대로 유지.
...
| 행 | 내용 | 쓰는 열 |
|---|---|---|
| 0~8 | source01~09 온도 | Max(8) → `max_temp`, Mean(9) → `temp_std` |
| 9 | Fan1_Passage 차압 | Mean(9) |
| 10~(10+n-1) | V_inlet_00~(n-1) (1차 통과, 유로 전체) 속도 | Mean(9) × Area(11) |
| (10+n)~(10+2n-1) | V_inlet2_00~(n-1) (2차 통과, 유로 전체) 속도 | Mean(9) × Area(11) |
| (10+2n) | Rectangle1 (전원모듈 분기 입구) 법선방향 속도 | Mean(9) × Area(11) |
...
### (1) 핀뱅크 길이 `FIN_SPAN_MM = 86.5` — 확정

V4(Code3) 형상을 역산하면 핀 6개·`t=2.5`·갭 7.357142857(피치 9.857142857)로
총길이 66.5mm였다. V5는 86.5mm(20mm 증가)를 전제한다 — 실물 대조 완료.

### (2) 핀뱅크 측정 기준좌표 (`icepak.py`) — 확정

`FIN_BANK1_Y_START = -91.24999998`, `FIN_BANK2_Y_START = 5.250000017`
(실측 반영 완료). V4 값(−101.25 / −4.749999983)에서 핀뱅크 길이가 20mm
늘어난 만큼 각각 +10mm 이동한 값이다.
